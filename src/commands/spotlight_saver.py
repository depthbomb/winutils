...
@app.register()
class SpotlightSaver(Command):
    name = 'spotlight-saver'
    description = 'Saves the Windows Spotlight lockscreen images to "Pictures\\Windows Spotlight"'
    options = [
        Opt('startup', default=False, help='Toggle executing this command on startup, then exits')
    ]
    ...
    def _toggle_startup(self) -> TriBool:
        ...
        if not IS_COMPILED:
            self.ctx.stdout.write('Startup can only be toggle when the app is compiled\n')
            return None
    ...
    def run(self, args):
        if args.get('startup', bool):
            startup_enabled = self._toggle_startup()
            if startup_enabled:
                self.ctx.stdout.write('Startup enabled\n')
            elif not startup_enabled:
                self.ctx.stdout.write('Startup disabled\n')
            else:
                self.ctx.stdout.write('Startup could not be toggled\n')
            return 0
